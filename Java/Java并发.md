...
wait() 和 sleep() 的区别
  ...
  - wait() 会释放锁，sleep() 不会。<strong>“wait会释放锁”的意思是，如果某个线程在一个`synchronized`代码块里被`wait`挂起，那么其他的线程就可以进入`synchronized`代码块了。`Lock`也是同理</strong>
...
多生产者-消费者模式如果不使用`while-notifyAll()`可能会出现问题，具体如下：
...
- 使用`if-notify`。
    - 初始状态`flag=false`，假如`t0`获得了`CPU`执行权进入`set`方法，判断`flag`标志不满足条件，所以不等待，执行生产操作生产“商品0”，将`count`加一并将`flag`设为`true`，之后进行一次`notify()`。此时“商品0”已经被生产，`count=1`。
    - 之后`t0`又获得了一次执行权，进入`set`方法后，发现`flag=true`满足等待条件，于是进入`wait`状态并释放锁。
    - 这时`t1`获得了执行权，进入`set`方法后也被`wait`挂起。
    - 然后`t2`获得了执行权，进入`out`方法后，执行消费操作，消费“商品0”，并将`flag`置为`false`，然后进行了一次`notify()`。假如唤醒的是`t0`。此时`flag=false`，只有`t1`在等待，“商品0”已经被消费。
    - 之后`t2`又获得了执行权，进入`out`方法后被挂起。
    - 然后`t3`获得了执行权，进入`out`后也被挂起。
    - 之后`t0`获得执行权，进入`set`方法后执行生产任务，生产“商品1”，并将`count`加一，然后将`flag`置为`true`并执行一次唤醒操作，假如`t1`被唤醒。此时“商品1”被生产，`count=2`。
    - 然后`t1`被唤醒之后获得了执行权，由于是`if`语句，所以`t1`不会再判断`flag`标志，而是直接从唤醒处（即`this.wait()`）直接往下执行，它生产了“商品2”。问题出现了，“商品1”永远不会被消费了，这就是“遗漏消费”。

- 使用`if-notifyAll`类似。

- 使用`while-notify`。
    - 初始状态`flag=false`，假如`t0`首先获得执行权，进入`set`方法之后，将`flag`置为`true`。然后进行了一次`notify`。
    - 之后`t0`又获得了执行权，发现`flag=true`满足条件，于是进入`wait`状态。
    - 然后`t1`获得了执行权，进入`set`方法后也被挂起。
    - 然后`t2`获得了执行权，进入`out`方法后，将`flag`置为`false`，并进行了一次`notify`，假如唤醒的是`t0`。
    - 然后`t2`又获得的执行权，进入`out`方法后被挂起。
    - 然后`t3`获得了执行权，进入`out`方法后被挂起。
    - 然后`t0`获得执行权，进入`set`方法后，将`flag`置为`true`，然后进行了一次`notify`，假如唤醒的是`t1`。
    - `t1`被换新之后，由于在`while`循环中，首先得判断一下`flag`，发现为`true`满足条件，又进入`wait`状态。
    - `t0`是唯一存活的线程，它获得了执行权，进入`set`后条件满足也被挂起，死锁发生。
...
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/68778c1b-15ab-4826-99c0-3b4fd38cb9e9.png" width="600">
</div>
...
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/47358f87-bc4c-496f-9a90-8d696de94cee.png" width="600">
</div>
...
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/536c6dfd-305a-4b95-b12c-28ca5e8aa043.png">
</div>
...
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/ef8eab00-1d5e-4d99-a7c2-d6d68ea7fe92.png" width="400">
</div>


AtomicInteger 能保证多个线程修改的原子性。
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/952afa9a-458b-44ce-bba9-463e60162945.png" width="400">
</div>
...
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/single-thread-rule.png" >
</div>
...
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/monitor-lock-rule.png" width="500">
</div>
...
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/volatile-variable-rule.png" width="500">
</div>
...
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/thread-start-rule.png" width="500">
</div>
...
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/thread-join-rule.png" width="500">
</div>
...
<strong>线程安全是指多个线程不管以何种方式访问某个类，并且在主调代码中不需要进行同步，都能表现正确的行为。</strong>
...
同步是指在多个线程并发访问共享数据时，保证共享数据在同一时刻只被一个(或者是一些，使用信号量的时候)线程使用。而互斥是实现同步的一种手段，临界区(Critical Section)、互斥量(Mutex)和信号量(Semaphore)都是主要的互斥实现方式。因此，<strong>互斥是因，同步是果；互斥是方法，同步是目的。</strong>
...
互斥同步最主要的问题就是线程阻塞和唤醒所带来的性能问题，因此这种同步也称为<strong>阻塞同步</strong>。
...
它所对应的底层结构图为：
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/3646544a-cb57-451d-9e03-d3c4f5e4434a.png">
</div>
...
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/bb6a49be-00f2-4f27-a0ce-4ed764bc605c.png" width="500">
</div>


下图左侧是一个线程的虚拟机栈，其中有一部分称为 Lock Record 的区域，这是在轻量级锁运行过程创建的，用于存放锁对象的 Mark Word。而右侧就是一个锁对象，包含了 Mark Word 和其它信息。
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/051e436c-0e46-4c59-8f67-52d89d656182.png" width="500">
</div>
...
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/baaa681f-7c52-4198-a5ae-303b9386cf47.png" width="500">
</div>
...
<div align="center">
<img src="https://raw.githubusercontent.com/adamhand/LeetCode-images/master/390c913b-5f31-444f-bbdb-2b88b688e7ce.jpg" width="500">
</div>
